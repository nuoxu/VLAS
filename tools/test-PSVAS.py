...
def test(epoch, search_budget=12): 
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'TEST_PSVAS_log.txt','a') as f:
...
agent = utils.Model_search_Arch_Adapt(num_grids=args.num_actions)
...
checkpoint = torch.load(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/PSVAS.pt")
