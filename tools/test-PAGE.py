...
#         target_node_index.append(torch.argmax(node_label[:, label_to_idx[cln]]))
...
def test(epoch, search_budget=12): 
    ...
    for batch_idx, (inputs, targets, classnames, numbers, chips) in tqdm.tqdm(enumerate(testloader), total=len(testloader)):
        ...
        for step_ in range(search_budget): 
            ...
            logit, grid_prob_, node_feat = search_agent.forward(inputs, classname_feat, chips_chosen, node_feat, edge, target_node_index, updated_search_info, loc_query_batch)
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'TEST_PAGE_log.txt','a') as f:
...
#------------------------------------------------------------------------------------------------------#
_, testset = utils.get_datasetPAGE(args.img_size, args.data_dir+'/'+args.dataset+'/', args.num_actions, args.multiclass)
...
checkpoint = torch.load(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/PAGE.pt")
