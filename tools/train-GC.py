"""
train the greedy classification Network
"""
import sys
sys.path.insert(0, '.')
import os
...
def train(epoch):
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'GC_log.txt','a') as f:
...
def test(epoch):
    ...
    torch.save(state, args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/GC.pt")
