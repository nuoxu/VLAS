...
def train(epoch):
    ...
    for batch_idx, (inputs, targets, classnames) in tqdm.tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        for step_ in range(search_budget):
            ...
            logit, grid_prob = agent.forward(inputs, classname_feat, updated_search_info, loc_query_batch) 
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'VLAS_log.txt','a') as f:
        f.write('Train: %d | Rw: %.2f \n' % (epoch, reward))
    
    print('Train: %d | Rw: %.2F | CE: %.2F | RL: %.2F' % (epoch, reward, loss_ce, loss_rl))

# test the agent's performance on VLAS setting    
def test(epoch, best_sr): 
    ...
    if (recall> best_sr):
        ...
        torch.save(state, args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/VLAS.pt")
...
trainset, testset = utils.get_datasetVLAS(args.img_size, args.data_dir+'/'+args.dataset+'/', args.num_actions, args.multiclass)
