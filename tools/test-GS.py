"""
test the Greedy Policy Network
"""
import sys
sys.path.insert(0, '.')
import os
...
def test(epoch, search_budget=12):
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'TEST_GS_log.txt','a') as f:
...
checkpoint = torch.load(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/GS.pt")
