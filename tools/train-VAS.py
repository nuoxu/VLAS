...
def train(epoch):
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'VAS_log.txt','a') as f:
...
def test(epoch, best_sr): 
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'VAS_log.txt','a') as f:
        f.write('Test - Recall: %.2f | SB: %.2f \n' % (recall, search_budget))
    ...
    if (recall> best_sr):
        ...
        torch.save(state, args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/VAS.pt") 
