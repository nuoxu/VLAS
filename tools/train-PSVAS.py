...
def train(epoch):
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'PSVAS_log.txt','a') as f:
...
def test(epoch, best_sr): 
    ...
    with open(args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+'/'+'PSVAS_log.txt','a') as f:
        f.write('Test - Recall: %.2f | ACC: %.2f | TPR: %.2f | SB: %.2f \n' % (recall, final_acc, final_tpr, search_budget))
    ...
    if (recall> best_sr):
        ...
        #torch.save(state, args.cv_dir+'/ckpt_E_%d_R_%.2F'%(epoch, success)) 
        torch.save(state, args.cv_dir+'/'+args.dataset+'/'+str(args.num_actions)+"/PSVAS.pt")
...
agent = utils.Model_search_Arch_Adapt(num_grids=args.num_actions)  #_
