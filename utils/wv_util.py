...
def get_labels_from_geojson(fname):
    """
...
def get_labels_from_txt(fname, label_name):
    """
    Gets label data from txt label files

    Args:
        fname: file path to dota txt label files
        label_name: name and index of labels
    ...
    f_dirs = os.listdir(fname)

    for f_dir in f_dirs:
        with open(os.path.join(fname, f_dir)) as f:
            lines = f.readlines()
        for i in tqdm(range(len(lines))):
            line = lines[i].split(' ')
            coords.append([float(line[0]),float(line[1]),
                           float(line[4]),float(line[5])])
            chips.append(f_dir.replace('.txt', '.png'))
            classes.append(float(label_name[line[8].replace('-', ' ')]))

    return np.array(coords), np.array(chips), np.array(classes)
...
def chip_image(img,coords,classes,shape=(300,300)):
    ...
    for i in range(w_num):
        for j in range(h_num):
            ...
            y = np.logical_or(np.logical_and((coords[:,1]<((j+1)*hn)),(coords[:,1]>(j*hn))),
                              np.logical_and((coords[:,3]<((j+1)*hn)),(coords[:,3]>(j*hn))))

            xx = np.logical_or(np.logical_and(((i*wn)<coords[:,2]),((i*wn)>coords[:,0])),
                               np.logical_and((((i+1)*wn)<coords[:,2]),(((i+1)*wn)>coords[:,0])))
            yy = np.logical_or(np.logical_and(((j*hn)<coords[:,3]),((j*hn)>coords[:,1])),
                               np.logical_and((((j+1)*hn)<coords[:,3]),(((j+1)*hn)>coords[:,1])))

            intersect = (x & y) | (xx & yy)
            out = coords[intersect]

            out = np.transpose(np.vstack((np.clip(out[:,0]-(wn*i),0,wn),
                                          np.clip(out[:,1]-(hn*j),0,hn),
                                          np.clip(out[:,2]-(wn*i),0,wn),
                                          np.clip(out[:,3]-(hn*j),0,hn))))
            box_classes = classes[intersect]
